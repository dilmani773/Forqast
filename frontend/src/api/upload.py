...
async def upload_sales(file: UploadFile = File(...)):
    ...
    # Average weekly units per dish — used as default price suggestion
    avg_weekly = {
        dish: round(float(group["units_sold"].mean()), 1)
        for dish, group in weekly.groupby("dish_name")
    }
    ...
    return UploadResponse(
        ...
        dish_histories   = dish_histories,
        avg_weekly_units = avg_weekly,
        message          = (
            f"Successfully processed {len(df):,} sales records "
            f"across {len(dishes_found)} dishes ({date_range})."
        ),
    )